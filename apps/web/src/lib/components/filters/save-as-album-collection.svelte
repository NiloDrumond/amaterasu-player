<script lang="ts">
	...
	let { filter }: { filter: FilterNode | null } = $props();
	...
	async function save() {
		...
		saving = true;
		const { data, error } = await createCollection(fetch, {
			name: name.trim(),
	...
	<Popover.Root bind:open>
		...
		<Popover.Content class="w-64 p-3" align="end">
			<div class="flex flex-col gap-2">
				<div class="text-xs font-medium">Save filters as a dynamic album collection</div>
				<Input
					bind:value={name}

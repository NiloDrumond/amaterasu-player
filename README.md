...
Add Loki + Grafana for structured logging and prebuilt dashboards. The overlay is self-contained — download it plus the two dashboard JSON files into the same folder as your `docker-compose.yml`:
...
curl -O https://codeberg.org/NiloDrumond/amaterasu-player/raw/branch/main/infra/grafana/dashboards/amaterasu-web-logs.json
docker compose -f docker-compose.yml -f docker-compose.monitoring.yml up -d
```

Grafana is accessible at <http://localhost:4534/admin/logs> (proxied through the app, requires admin login). It is not directly exposed — only reachable through the app's reverse proxy, and your admin login is your Grafana login. Two dashboards are provisioned under the **Amaterasu** folder: **Amaterasu server logs** (backend) and **Amaterasu web logs** (frontend errors and client-side logs).

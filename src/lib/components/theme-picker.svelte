...
>
  <button class="solid" on:click={() => (open = !open)}
    ><Icon width={18} height={18} icon={getIcon($theme)} /></button
  >
  {#if open}
    <div
      class="absolute bg-slate-100 rounded-xl border border-slate-400 -right-4 top-[calc(100%_+_0.25rem)] overflow-hidden"
    >
      <button class="menu-item" on:click={() => theme.setTheme('dark')}
        ><Icon width={18} height={18} icon={themeIcons.dark} />Dark</button
      >
      <button class="menu-item" on:click={() => theme.setTheme('light')}
        ><Icon width={18} height={18} icon={themeIcons.light} />Light</button
      >
      <button class="menu-item" on:click={() => theme.setTheme('system')}
